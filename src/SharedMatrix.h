#ifndef __SHARED_MATRIX_H_
#define __SHARED_MATRIX_H_
...
class SharedMatrix {
	public:
		...
		unsigned int width,height;          // width and height of the block of doubles
		SharedMatrix () {
			cached_source = "";
			//operation = "";
			shmem_name = "";
		...
		void fromCache (const std::string cached_source_in);
		void Cache ();

		int New( unsigned int w, unsigned int h, std::string matrix_name );
	...
	// Overrides of parent class methods
		...
		virtual ~SharedMatrix();                                 // destructor
	...
	private:
		...
		static const size_t calc_shmem_size (const unsigned int w, const unsigned int h, size_t &clr_plane_offset, size_t &shmem_data_offset);
