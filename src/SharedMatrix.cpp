...
size_t SharedMatrix::shmem_page_size = sysconf(_SC_PAGE_SIZE);
bool SharedMatrix::never_read = false;
bool SharedMatrix::disable_destructor_cache_cleanup = false;
pid_t SharedMatrix::PID = getpid();
std::string SharedMatrix::PID_string = string_format ("%lld",(long long)SharedMatrix::PID);


void SharedMatrix::SetShmemName ( ) {
	MD5::MD5 digest;
	...
	assert (!(cached_source.empty()) && "Attempt to establish a shared memory name without a cached_source");
	...
	std::cout <<         "         SharedMatrix::SetShmemName: " << shmem_name << std::endl;
...
//CEC edit
//const size_t SharedMatrix::calc_shmem_size (const unsigned int w, const unsigned int h, const enum ColorModes ColorMode, size_t &clr_plane_offset, size_t &shmem_data_offset) {
const size_t SharedMatrix::calc_shmem_size (const unsigned int w, const unsigned int h, size_t &clr_plane_offset, size_t &shmem_data_offset) {
...
void SharedMatrix::allocate (unsigned int w, unsigned int h) {
 	std::cout << "-------- called SharedMatrix::allocate (" << w << "," << h << ") on " << shmem_name << std::endl;
	...
	new_shmem_size = calc_shmem_size (w, h, clr_plane_offset, shmem_data_offset);
...
// cache_read means the SharedMatrix was read in from cache.
// cache_write means the SharedMatrix has exclusive write access.
...
//void SharedMatrix::fromCache (const std::string cached_source_in, const std::string operation_in) {	
void SharedMatrix::fromCache (const std::string cached_source_in) {	
...
std::cout <<         "-------- called SharedMatrix::fromCache on [" << cached_source << std::endl; // "]->[" << operation << "]" <<std::endl;;
	SetShmemName();
...
std::cout << "cache_read" << std::endl;
			assert (!never_read && "SharedMatrix Class is set to never read, but the lockfile still exists after unlinking it!");
...
std::cout << "shmem_size: " << shmem_size << std::endl;
			...
			// This will tell us how to read the matrixes and reconstruct the SharedMatrix
...
std::cout <<         "-------- called SharedMatrix::Cache on [" << cached_source << std::endl;//"]->[" << operation << "]" <<std::endl;;
	SetShmemName();
...
/*int SharedMatrix::OpenImage(char *image_file_name,
		int downsample, rect *bounding_rect,
		double mean, double stddev) {
*/
int SharedMatrix::New( unsigned int w, unsigned int h, std::string matrix_name ) {

	width = w;
	height = h;

	//std::string open_operation( matrix_name );
...
// Of course this returns a new SharedMatrix as an ImageMatrix pointer.
// We have to declare the virtual method this way to make the override work.
/*void SharedMatrix::transform (const SharedMatrix &matrix_IN, const ImageTransform *transform) {
...
std::cout << "SharedMatrix DESTRUCTOR for " << shmem_name << std::endl;
...
std::cout << "    unlinking POSIX shared memory " << shmem_name << std::endl; // " and lockfile " << lock_file.path << std::endl;

		if( shm_unlink (shmem_name.c_str()) != 0 )
			std::cout << "error unlinking shared memory: " << strerror(errno) << " (errno : " << errno << ")\n";

		//if( unlink (lock_file.path.c_str()) != 0 )
			//std::cout << "error unlinking file: " << strerror(errno) << " (errno : " << errno << ")\n";
	}
